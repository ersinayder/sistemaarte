...
const COLUNAS = [
  ...
  { status: 'Em Produção',  label: 'Em Produção',  bg:'rgba(0,100,148,0.12)',        color:'var(--color-blue)' },
          ...
          {COLUNAS.map(col => {
            ...
            return (
              <div key={col.status}
                ...
                style={{ flex:'1 1 260px', minWidth:220, maxWidth:380, display:'flex', flexDirection:'column',
                  ...
                  return (
                    <tr key={o.id} onClick={() => navigate(`/ordens/${o.id}`)}
                      ...
                      <td style={{ padding:'var(--space-2) var(--space-3)' }}>
                        <span className={`badge badge-${o.status==='Pronto'?'success':o.status==='Em Produção'?'blue':'secondary'}`} style={{ fontSize:10 }}>{o.status}</span>
                      </td>
                      <td style={{ padding:'var(--space-2) var(--space-3)', fontSize:'var(--text-xs)',
