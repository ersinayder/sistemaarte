...
const statusColor = s => ({
  'Aguardando':'secondary','Em Producao':'blue','Pronto':'success','Entregue':'primary','Cancelado':'danger'
}[s]||'secondary');
...
function ModalOS({ os, onClose, onSaved, clientes, todosProdutos, canEdit, canEditFinanceiro }) {
  ...
  const STATUS_OPTS = ['Aguardando','Em Producao','Pronto','Entregue','Cancelado'];
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
      ...
      <div style={{ padding:'var(--space-3) var(--space-6)', display:'flex', gap:'var(--space-3)',
        ...
        <select className="form-input" value={filterStatus} onChange={e=>setFilterStatus(e.target.value)}
          ...
          {['Aguardando','Em Producao','Pronto','Entregue','Cancelado'].map(s=><option key={s}>{s}</option>)}
        </select>
