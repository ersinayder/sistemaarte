...
export default function Caixa() {
  ...
  const handleSave = async (form) => {
    try {
      if (editItem) {
        await api.put(`/lancamentos/${editItem.id}`, form);
        ...
        <div style={{ display:'flex', gap:'var(--space-2)', alignItems:'center' }}>
          ...
          {viewMode === 'dia' && (
            <button className="btn btn-secondary"
              ...
              onClick={() => gerarPDFFechamento(lancamentosDia, selectedDay, diaEntrada, diaSaida, diaSaldo)}>
            ...
            {paginated.length === 0 ? (
              ...
              <tr key={l.id} style={{ borderBottom:'1px solid var(--color-border)' }}>
                ...
                <td style={{ padding:'var(--space-2) var(--space-3)', color:'var(--color-primary)', fontWeight:600 }}>
                  {l.os_numero ? <span style={{ cursor:'pointer' }}>{l.os_numero}</span> : '—'}
                </td>
...
function ModalLancamento({ item, onClose, onSave, defaultDate }) {
  ...
  const handleOsChange = (e) => {
    ...
    if (!id) { set('os_id',''); set('os_numero',''); set('descricao',''); return; }
    const os = ordens.find(o => String(o.id) === String(id));
    if (os) {
      set('os_id', os.id);
      set('os_numero', os.numero);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onSave({ ...form, valor: Number(form.valor)||0, os_id: form.os_id||null });
  };
          ...
          <div style={{ padding:'var(--space-4) var(--space-5)', display:'flex', flexDirection:'column', gap:'var(--space-3)' }}>
            ...
            {form.tipo === 'Entrada' && (
              <div>
                ...
                <select className="form-input" value={form.os_id} onChange={handleOsChange}
