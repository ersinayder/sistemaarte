...
- Fase 6 concluida: `/api/permission-profiles` expoe perfis e catalogo de permissoes, a tela `/usuarios` ganhou aba `Perfis` com matriz por modulo, alteracoes de perfil invalidam sessoes afetadas, permissoes desconhecidas sao rejeitadas e perfis padrao podem ser restaurados explicitamente.
- A edicao da matriz de permissoes exige `configuracoes.seguranca`; o perfil `admin` permanece protegido com acesso total e o backend bloqueia mudancas que deixariam o sistema sem usuario ativo capaz de gerenciar usuarios.
- Fase 7 concluida: perfis customizados podem ser criados em `/api/permission-profiles` e atribuidos pela tela `/usuarios`, separando `users.role` como tipo estrutural fixo (`admin`, `caixa`, `oficina`) e `users.profile_key` como perfil efetivo de permissao.
- `permission_profiles.base_role` define compatibilidade entre perfil customizado e tipo estrutural; o backend exige perfil existente, ativo e compativel antes de gravar `profile_key`, incrementa `access_version` em mudancas de acesso e bloqueia remocoes que deixariam o sistema sem usuario ativo capaz de gerenciar usuarios.
